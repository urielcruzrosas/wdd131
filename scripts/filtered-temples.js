...
function displayFilteredTemples(filteredArray) {
    gridContainer.innerHTML = '';

    filteredArray.forEach(temple => {
        const card = document.createElement('figure');
...
navMenuLinks.forEach(link => {
    link.addEventListener('click', (e) => {
        ...
        navMenuLinks.forEach(navLink => navLink.classList.remove('active'));
        link.classList.add('active');

        const filterType = link.textContent;

        if (filterType === 'Old') {
            displayFilteredTemples(temples.filter(temple => parseInt(temple.dedicated.substring(0, 4)) < 1900));
        }
        else if (filterType === 'New') {
            displayFilteredTemples(temples.filter(temple => parseInt(temple.dedicated.substring(0, 4)) > 2000));
        }
        else if (filterType === 'Large') {
            displayFilteredTemples(temples.filter(temple => temple.area > 90000));
        }
        else if (filterType === 'Small') {
            displayFilteredTemples(temples.filter(temple => temple.area < 10000));
        }
        else {
